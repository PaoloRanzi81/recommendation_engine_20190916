...
#input_file_name_2 = ('re_scores_clean.csv')
...
#analysis = 'grid_search' # whenever starting the script from the Linux bash, uncomment such variable
...
# pop out 'RE'
y = re_scores_tmp.pop('event.type')    
    ...
    def grid_search(model, param_grid, X, y):    
        ...
        if RELEASE == '5.0.0-31-generic': # Linux laptop
            ...
            grid = GridSearchCV(estimator = model, param_grid = param_grid,
                                ...
                                scoring = scoring, refit='f1_micro')
...
if analysis == 'grid_search': 
    ...
    for i in range(n_iterations):
        ...
        # set cross-validation  
        cv = StratifiedShuffleSplit(n_splits = int(round((cpu_count() - 1), 0)), test_size=0.2, 
                                    random_state = None)
        ...
        if RELEASE == '5.0.0-31-generic': # Linux laptop
            ...
            grid = GridSearchCV(estimator = model, param_grid = param_grid,
                         iid = True, cv = cv,
                         n_jobs = int(round((cpu_count() - 1), 0)), scoring = scoring,
                         refit='matthews_corrcoef')
